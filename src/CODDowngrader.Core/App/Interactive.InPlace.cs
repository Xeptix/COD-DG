...
public sealed partial class Interactive
{
        ...
        IReadOnlyDictionary<uint, uint> owners, string label, string tag)
    {
        ...
        if (PatchStore.AppliedTo(app.InstallDir) is { } written) MadeBuilds.Add(MadeBuilds.FromApplied(written, game, "ingame"));
        ...
        IReadOnlyDictionary<uint, uint> owners, string label, string tag)
    {
        ...
        PatchStore.SavePatch(folder, record);
        MadeBuilds.Add(MadeBuilds.FromPatch(record, game, folder));
    ...
    async Task ApplyFolderAsync(GameLibrary library, GameEntry game)
    {
        ...
        if (PatchStore.AppliedTo(app.InstallDir) is { } applied) MadeBuilds.Add(MadeBuilds.FromApplied(applied, game, "apply", folder));
