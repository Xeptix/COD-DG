...
public sealed partial class Interactive
{
    ...
    {
        AnsiConsole.MarkupLine("[grey]One per line: a download_depot command, or a depot ID and a manifest ID, or a shared build. An empty line finishes.[/]");
        while (Console.ReadLine() is { } read && read.Trim().Length > 0)
        {
            // A shared build pastes in whole: its manifest lines are depot=manifest, and the rest of it is for the window and commands.
            var line = read.Trim().Trim('`').Trim();
            if (line.Length == 0 || line.StartsWith(Builds.SharedBuild.Header, StringComparison.OrdinalIgnoreCase)) continue;
            var word = line.Split(' ', 2)[0].ToLowerInvariant();
            if (word is "game" or "title" or "only" or "file" or "siblings") continue;
            if (word == "manifest") line = line["manifest".Length..];
            ...
            library.Remembered.Learn(new[] { new CODDowngrader.Catalog.RememberedManifest(depot, manifest, CODDowngrader.Catalog.RememberedSource.Named) });
            origin[depot] = "pasted";
        }
    }
    ...
    static string ManifestLabel(KnownManifest k)
    {
        ...
        if (k.Remembered) parts.Add("remembered");
