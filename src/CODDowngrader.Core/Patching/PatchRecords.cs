...
public static class PatchStore
{
    ...
    /// <summary>Every downgrade written into a game on this PC.</summary>
    public static List<AppliedRecord> AllApplied(string? stateFolder = null)
    {
        var folder = stateFolder ?? AppliedFolder;
        if (!Directory.Exists(folder)) return new List<AppliedRecord>();
        try
        {
            return Directory.EnumerateFiles(folder, "*.json")
                .Select(path => Load(path, PatchJson.Default.AppliedRecord))
                .OfType<AppliedRecord>()
                .ToList();
        }
        catch (Exception e) when (e is IOException or UnauthorizedAccessException)
        {
            return new List<AppliedRecord>();
        }
    }
