...
public sealed class DepotDownloaderResult
{
    ...
    /// <summary>
    /// DepotDownloader asked for a password or a Steam Guard code in a run that had nobody to answer, and was stopped. The
    /// saved sign-in no longer works: a sign-in window is what fixes it.
    /// </summary>
    public bool AskedToSignIn { get; set; }
...
public static partial class DepotDownloaderTool
{
    ...
    // No packager publishes the Linux zip's hash (GitHub's digest is null, nixpkgs builds from source), so it was taken from
    // the release download on 2026-09-16, after checking its size against GitHub's and that the binary carries
    // 3.4.0+c553ef4d60c00a4f5fd16c9fe017f569001589ff, the same build as the Windows zip above.
    static readonly DepotDownloaderRelease LinuxX64 =
        new("DepotDownloader-linux-x64.zip", 33_442_357, "A999DEC66B4850FC961BD50366696D23C2D0FAD7B18790E6A5647B2F19097A53");
    ...
    /// <summary>What DepotDownloader and SteamKit print when they wait for someone to type a password or a Steam Guard code.</summary>
    static readonly string[] SignInPrompts =
    {
        "Enter account password", "Please enter", "STEAM GUARD", "Cannot read keys",
    };

    internal static bool IsSignInPrompt(string text) => SignInPrompts.Any(p => text.Contains(p, StringComparison.OrdinalIgnoreCase));
    ...
    /// Whether someone can answer DepotDownloader here. When not (the window, a program reading JSON), it runs with no
    /// console window and no input, and a password or Steam Guard prompt stops it with
    /// <see cref="DepotDownloaderResult.AskedToSignIn"/> instead of waiting for an answer nobody can give.
    /// </param>
    public static async Task<DepotDownloaderResult> RunAsync(string exe, IReadOnlyList<string> arguments, string logPath,
        CancellationToken ct = default, TextWriter? mirror = null, bool interactive = true)
    {
        ...
        var psi = new ProcessStartInfo(exe)
        {
            ...
            // A program with no console of its own, the window among them, would otherwise open an empty console window per run.
        ...
        try
        {
            ...
            if (!interactive)
            {
                // Nothing will be typed: a prompt reads the end of input at once instead of waiting.
                process.StandardInput.Close();
                onPrompt = () =>
                {
                    if (result.AskedToSignIn) return;
                    result.AskedToSignIn = true;
                    Kill();
                };
            }
            var stdout = Pump(process.StandardOutput, mirror ?? Console.Out, () => log, v => log = v, gate, result, onPrompt);
            var stderr = Pump(process.StandardError, mirror ?? Console.Error, () => log, v => log = v, gate, result, onPrompt);
    ...
    static async Task Pump(StreamReader reader, TextWriter console, Func<StreamWriter?> getLog, Action<StreamWriter?> setLog, object gate,
        DepotDownloaderResult result, Action? onPrompt)
    {
        ...
        while ((read = await reader.ReadAsync(buffer, 0, buffer.Length)) > 0)
        {
            ...
            for (var i = 0; i < read; i++)
            {
                ...
                var complete = line.ToString().TrimEnd('\r');
                Record(complete, getLog, setLog, gate, result);
                if (onPrompt is not null && IsSignInPrompt(complete)) onPrompt();
                line.Clear();
            }

            // A prompt ends without a newline, so the line so far is checked too.
            if (onPrompt is not null && line.Length > 0 && IsSignInPrompt(line.ToString())) onPrompt();
        }
