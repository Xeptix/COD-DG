...
/// <summary>A way Steam starts an app: the program, relative to the install folder, and the systems it is for.</summary>
public sealed record LaunchOption(string Executable, string? OsList, string? Type)
{
    public bool ForWindows => string.IsNullOrEmpty(OsList) || OsList.Contains("windows", StringComparison.OrdinalIgnoreCase);
}
    ...
    IReadOnlyList<DepotInfo> Depots)
{
    ...
    /// <summary>How Steam starts the app, in the order its product info lists them.</summary>
    public IReadOnlyList<LaunchOption> Launch { get; init; } = Array.Empty<LaunchOption>();

    /// <summary>The name of the app's icon in Steam's library cache, a hash.</summary>
    ...
    public static AppInfo From(uint appId, KvNode app)
    {
        ...
        var launch = new List<LaunchOption>();
        foreach (var option in app["config"]?["launch"]?.Children ?? new List<KvNode>())
            if (option.Get("executable") is { Length: > 0 } executable)
                launch.Add(new LaunchOption(executable, option["config"]?.Get("oslist"), option.Get("type")));

        var pub = depotsNode?["branches"]?["public"];
        return new AppInfo(appId, app["common"]?.Get("name"), ParseUInt(pub?.Get("buildid")), depots) { Branches = branches, Launch = launch, Icon = app["common"]?.Get("icon") };
    }
