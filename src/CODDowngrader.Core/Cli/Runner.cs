...
public static class Runner
{
    ...
    public static async Task<bool> SignInAsync(Job run, LoginState login)
    {
    ...
    /// </summary>
    public static async Task<RunOutcome> DepotsAsync(Job run, string exe, uint appId, IReadOnlyList<(uint Depot, ulong Manifest)> depots,
        IReadOnlyDictionary<uint, uint> owners, string directory, string logPath, LoginState login, string? fileList = null, bool manifestOnly = false)
    {
                     ...
                     .ThenBy(g => g.Key))
        {
            for (var attempt = 1; ; attempt++)
            {
                ...
                run.Progress(new JobProgress(manifestOnly ? "Fetching file lists" : "Downloading", $"{group.Count()} depots of app {group.Key}", manifestOnly ? null : 0));
                var arguments = DepotDownloaderTool.Arguments(group.Key, group.ToList(), directory, login.Account, fileList, manifestOnly);
                var result = await DepotDownloaderTool.RunAsync(exe, arguments, logPath, run.Cancel, mirror: run.ToolOutput, interactive: run.CanPromptHere);
    ...
    /// </summary>
    public static async Task<Dictionary<(uint Depot, ulong Manifest), IReadOnlyList<ManifestFile>>?> ListsAsync(Job run, string exe,
